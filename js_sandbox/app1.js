...
val = (5).toString();
//boolean
val = (true).toString(); // can use either stringify method

//String to number!
val = Number('5');
val = Number(true); //parse true gives us one
val = Number(false);
val = Number(null);
val = Number('hello'); //will give a NaN
val = Number([1, 2, 3]);

//parseInt
val = parseInt('100.30'); //will parse as an integer, if we want decimales use pars float
...
//Type Coercion, one type changed to another Javascript changes types for us, type coercion needs to be looked for.
const val1 = String(5);
const val2 = 6;
